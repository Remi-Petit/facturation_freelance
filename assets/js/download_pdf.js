...
function downloadPDF(numFact){
  ...
  let prixTTC = parseFloat(myFacture.coutHT)*1.2;
  let prixTTC1 = prixTTC.toString();
  ...
  doc.setFontSize(12);
  doc.setFontType("bold");
  doc.text("Intitulé", 20, 148);
  doc.setFontType("bold");
  doc.text("Tarif HT (EUR)", 75, 148);
  doc.setFontType("bold");
  doc.text("Tarif TTC (TVA à 20%)", 120, 148);
  ...
  doc.setFontSize(12);
  doc.setFontType("normal");
  doc.text("Création de maquettes :", 20, 158);
  doc.setFontType("normal");  
  doc.text(myFacture.coutHT + "€", 75, 158);
  doc.text(prixTTC1 + "€", 120, 158);
